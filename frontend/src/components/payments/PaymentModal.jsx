...
export default function PaymentModal({ isOpen, onClose, dossier, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-white dark:bg-slate-950 w-full max-w-lg rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Enregistrer un paiement</h3>
              <p className="text-xs text-slate-400">Guichet communal d'état civil</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors focus:outline-none"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800/60 space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-slate-400 font-medium">Citoyen</span>
              <span className="font-semibold text-slate-700 dark:text-slate-200">{citizenName}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-slate-400 font-medium">Demande</span>
              <span className="font-semibold text-slate-700 dark:text-slate-200 font-mono">{dossier.reference}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-slate-400 font-medium">Service</span>
              <span className="font-semibold text-slate-700 dark:text-slate-200">{getDossierTypeLabel(dossier.type)}</span>
            </div>
          </div>

          {/* Montant */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Montant à payer (XOF)</label>
            <Input
              type="number"
              value={amount}
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Mode de paiement</label>
          ...
          {isMobilePayment && (
            <div className="space-y-1.5 animate-in slide-in-from-top-2 duration-150">
              ...
              <Input
                ...
                required
              />
              <p className="text-[10px] text-slate-400">Obligatoire pour le suivi du paiement mobile.</p>
            </div>
          )}

          {/* Commentaire */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Commentaire (Optionnel)</label>
            <textarea
